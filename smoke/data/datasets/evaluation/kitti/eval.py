...
def eval_class(gt_annos,
    ...
    """
    print('gt len: ', len(gt_annos))
    print('dt len: ', len(dt_annos))
    assert len(gt_annos) == len(dt_annos)
    num_examples = len(gt_annos)
    split_parts = get_split_parts(num_examples, num_parts)
    ...
    precision = np.zeros(
        [num_class, num_difficulty, num_minoverlap, N_SAMPLE_PTS])  # (num_class, 3, 2, 41)
    recall = np.zeros(
        ...
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for i in range(len(thresholds)):
                    ...
                    # print('precision:', precision[m, l, k, i])
                    if compute_aos:
...
def get_official_eval_result(gt_annos,
    ...
    overlap_easy = np.array([[0.7, 0.5, 0.5, 0.7, 0.5, 0.5, 0.5, 0.5],
                            ...
                            [0.5, 0.25, 0.25, 0.5, 0.25, 0.5, 0.5, 0.5]])
    min_overlaps = np.stack([overlap_mod, overlap_easy], axis=0)  # [2, 3, 8]
    ...
    for j, curcls in enumerate(current_classes):
        print('{}/{}'.format(j, len(current_classes)))
